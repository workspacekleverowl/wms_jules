...
@php
    ...
    $perPage = 7; // Changed to 7 items per page
    $totalItems = count($items);
    
    // Calculate pages based on the new logic
    if ($totalItems == 0) {
        $pages = 1;
    } else {
        ...
        <table>
            ...
            <tbody>
                @foreach($pageItems as $i => $item)
                    @php $serialNumber = $start + $i + 1; @endphp
                    <tr>
                ...
                {{-- No blank rows needed for regular item pages --}}
            </tbody>
        </table>
    </div>
    @endif
