...
<head>
    ...
    <title>Scrap Return Report</title>
    <style>
        @page { 
            margin: 20px 25px; 
                ...
                <table class="table">
                    <thead class="table-header">
                        <tr>
                            ...
                            @if ($showValues)
                            <th class="header-cell" style="width: 12%;">Scrap Due(Kg)</th>
                            <th class="header-cell" style="width: 12%;">Scrap Given(Kg)</th>
                    ...
                    <tbody>
                        @forelse ($chunk as $record)
                            <tr class="{{ $loop->iteration % 2 === 0 ? 'striped-dark' : 'striped-light' }} {{ $record['voucher_type'] == 'opening' ? 'bold' : '' }}">
                                ...
                                @if ($showValues)
                                <td class="cell text-right">{{ number_format($record['scrap_due'], 3) }}</td>
                                <td class="cell text-right">{{ number_format($record['scrap_given'], 3) }}</td>
                        ...
                        @if ($isLastRecordPage)
                            <tr class="footer-row">
                                ...
                                <td></td>
                                @if ($showValues)
                                <td class="cell text-right bold">{{ number_format($data['summary']['scrap_due'], 3) }}</td>
                                <td class="cell text-right bold">{{ number_format($data['summary']['scrap_given'], 3) }}</td>
            ...
            @if (($isLastRecordPage && !$addExtraSummaryPage) || $isSummaryOnlyPage)
                ...
                <div class="summary-box">
                    ...
                    <table class="summary-table" style="width:100%;">
                         @if ($showValues)
                         <tr>
                            <td><strong>Total Scrap Due:</strong> {{ number_format($data['summary']['scrap_due'], 3) }}</td>
                            <td><strong>Total Scrap Given:</strong> {{ number_format($data['summary']['scrap_given'], 3) }}</td>
                         </tr>
                         <tr>
                         ...
                         @endif
                         @if (($data['userSettings']['scrap_inhouse_mr'] ?? 'no') === 'yes')
                         <tr>
                         ...
                         @endif
                         @if (($data['userSettings']['scrap_inhouse_cr'] ?? 'no') === 'yes')
                         <tr>
                         ...
                         @endif
                         @if (($data['userSettings']['scrap_inhouse_bh'] ?? 'no') === 'yes')
                         <tr>
